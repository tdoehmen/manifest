...
class HFInferenceAPIClient(Client):
    """HuggingFace Inference API client."""

    # Params are defined in https://huggingface.co/docs/api-inference/en/tasks/chat-completion
    PARAMS = {
    ...
    def connect(
        ...
        """
        self.api_key = connection_str or os.environ.get("HF_TOKEN")
        if self.api_key is None:
            raise ValueError(
                "HuggingFace Token not set. Set HF_TOKEN environment "
                "variable or pass through `client_connection`."
            )
        self.host = "https://api-inference.huggingface.co/"
    ...
    def get_generation_url(self) -> str:
        ...
        return (
            self.host
            + f"models/{self.get_model_params()['engine']}/v1/chat/completions"
        )
